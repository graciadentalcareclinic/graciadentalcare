...
const whatsappNumber = '6289637507810'; // Use the same number as appointment
...
const menuItems = [
  ...
  },
  {
    label: 'Appointment',
    icon: (
      <svg className="floating-menu-icon" width="20" height="20" fill="none" viewBox="0 0 24 24"><rect width="18" height="16" x="3" y="5" fill="#0ea5e9" rx="2"/><path fill="#fff" d="M7 9h2v2H7V9Zm4 0h2v2h-2V9Zm4 0h2v2h-2V9Z"/></svg>
    ),
    to: '/appointment',
    color: 'bg-white',
    isAppointment: true,
...
const BottomMenu = ({ onAppointmentClick, onServicesClick, onPromoClick }: FloatingMenuProps) => {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 flex justify-around items-center py-2 bg-white shadow-2xl border-t border-gray-200 md:hidden">
      ...
      <button className="flex flex-col items-center flex-1 focus:outline-none" onClick={onAppointmentClick}>
        <svg className="floating-menu-icon" width="20" height="20" fill="none" viewBox="0 0 24 24"><rect width="18" height="16" x="3" y="5" fill="#0ea5e9" rx="2"/><path fill="#fff" d="M7 9h2v2H7V9Zm4 0h2v2h-2V9Zm4 0h2v2h-2V9Z"/></svg>
        <span className="text-xs mt-1 font-semibold text-sky-500" style={{ fontFamily: 'inherit' }}>Appointment</span>
      </button>
      <button className="flex flex-col items-center flex-1 focus:outline-none" onClick={onPromoClick}>
        <svg className="floating-menu-icon" width="20" height="20" fill="none" viewBox="0 0 24 24"><path fill="#0ea5e9" d="M20.59 13.41 12 22l-8.59-8.59a2 2 0 0 1 0-2.82l7.59-7.59a2 2 0 0 1 2.82 0l7.59 7.59a2 2 0 0 1 0 2.82ZM7 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4Z"/></svg>
...
function FloatingMenu({ onAppointmentClick, onServicesClick, onPromoClick }: FloatingMenuProps) {
  ...
  return isMobile
    ? <BottomMenu onAppointmentClick={onAppointmentClick} onServicesClick={onServicesClick} onPromoClick={onPromoClick} />
    : <SideMenu onAppointmentClick={onAppointmentClick} onServicesClick={onServicesClick} onPromoClick={onPromoClick} />;
}
